...
class ExampleRestfulView(RestfulView):
    _validated = False
    _get = False
...
class TestRestfulView(ViewFixtureMixin):
    ...
    def view(self, mroot_factory, mrequest):
        return ExampleRestfulView(mroot_factory, mrequest)
    ...
    def test_view_flow(self, view, mrequest):
        """
        .view should first run .validate and after that proper method indicated
        by the request method.
        """
        mrequest.method = 'GET'

        assert view() == sentinel.RESULT
        assert view._validated
        assert view._get
